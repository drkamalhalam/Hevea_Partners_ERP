 ...
 * It performs direct field reads and simple formatting only.
 *
...
/** Format a number as Indian Rupee string with commas */
...
function formatLandArea(area: number | null | undefined, unit: string): string | null {
...
export async function resolveAgreementVariables(
  ...
  for (const def of Object.values(VARIABLE_REGISTRY)) {
    ...
    const value = resolveVariable(def.name, agreement, project, landOwner, developer);
    results.push({
      name: def.name,
      value,
...
function resolveVariable(
  ...
  switch (name) {
    ...
    case "PROJECT_LOCATION":
      return (project as any)?.location ?? null;
    ...
    case "DATE":
      return agreement.executionDate ?? null;
    ...
    case "LAND_AREA":
      return formatLandArea(agreement.landArea, agreement.landAreaUnit);
    case "OWNERSHIP_SHARE":
      return formatPercent(agreement.ownershipShareLandowner);
    case "DEVELOPER_OWNERSHIP_SHARE":
      return formatPercent(agreement.ownershipShareDeveloper);
    ...
    case "YEARLY_ESCALATION":
      return agreement.yearlyEscalation != null
        ? `${agreement.yearlyEscalation}%`
        : null;
    case "REVENUE_MODEL":
